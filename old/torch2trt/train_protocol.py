...
def train(model_params,outputfolder,gpuid):
        ...
        trainer.add_loss(LOSS_PINN(FHN_LOSS,batch_gen,batch_size=4*1024),weigth=1)
        ...
        trainer.add_loss(FHN_LOSS_fromODE(ode,[0,tf],ICs,batch_size=1024,num_points=102400,device=device,dtype=dtype))

        ##Validator
        trainer.add_validator(FHN_VAL_fromODE(ode,[0,tf],ICs,1024,device=device,name="val",dtype=dtype,dump_factor=1))
