...
dir="batch_results2/simulation1"
...
batch_size=102400*10
# create example data
x = torch.ones((batch_size,4)).cuda()
...
# Clear GPU memory
torch.cuda.empty_cache()
ts=[int(batch_size*x) for x in range(2,4)]
tcs,tns,tes=[],[],[]
...
if True:
    ...
    for T in ts:
            ...
            cuda_time=0
            for i in range(nrp):
                    cuda_time_a,x0,un,uref=runCuda(sampleset,batch_size=batch_size,dt=.1,rate=1)
            ...
            net_time=0
            for i in range(nrp):
                pr,net_time_a=runModel(x0,batch_size=batch_size)
            ...
            net_time=0
            for i in range(nrp):
                pr,net_time_a=runModel(x0,M=model_trt,batch_size=batch_size)
                net_time=net_time+net_time_a/nrp
